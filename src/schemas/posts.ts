...
export const posts = collection({
  ...
  schema: {
    ...
    category: fields.select({
      ...
      options: [{ label: t('ui-admin', 'categories.no-category.label'), value: 'p' }, ...CATEGORIES],
      defaultValue: 'p'
    }),
    tags: TAGS
      ? fields.multiselect({
        ...
        CloudImage: block({
          ...
          schema: {
            src: fields.text({
              ...
              description: t('ui-admin', 'posts.body.cloud-image.src.description'),
              validation: { length: { min: 1, max: 1275 } }
            }),
            alt: fields.text({
              label: t('ui-admin', 'posts.body.cloud-image.alt.label'),
              description: t('ui-admin', 'posts.body.cloud-image.alt.description')
            }),
            figcaption: fields.text({
              label: t('ui-admin', 'posts.body.cloud-image.figcaption.label'),
              description: t('ui-admin', 'posts.body.cloud-image.figcaption.description')
            })
          }
        }),
        Gallery: block({
            ...
            images: fields.array(
              fields.object({
                ...
                width: fields.integer({
                  ...
                  description: t('ui-admin', 'posts.body.gallery.images.width.description'),
                  validation: {
                    min: 150,
                    max: 3000
                  }
                ...
                height: fields.integer({
                  label: t('ui-admin', 'posts.body.gallery.images.height.label'),
                  description: t('ui-admin', 'posts.body.gallery.images.height.description'),
                  validation: {
                    min: 150,
                    max: 3000
                  }
