...
const SparkleButton = ({ text, size, variant }: SparkleProps) => {
  ...
  useEffect(() => {
    ...
    particlesRef.current.forEach((P) => {
      if (P) {
        ...
        P.style.setProperty('--origin-x', `${Math.random() > 0.5 ? RANDOM(300, 800) * -1 : RANDOM(300, 800)}%`)
        P.style.setProperty('--origin-y', `${Math.random() > 0.5 ? RANDOM(300, 800) * -1 : RANDOM(300, 800)}%`)
  ...
  return (
    <div className="flex items-center justify-center w-full h-full bg-transparent overflow-hidden">
      <div className="sparkle-button mx-20 my-20 relative">
        <Button
          className="relative overflow-visible mx-auto my-auto visible:full text-2xl text-sm py-3 px-5 rounded-full flex items-center gap-1 whitespace-nowrap transition-all duration-250 cursor-pointer"
        ...
        >
          <span
            ...
            style={{
              background:
                'linear-gradient(90deg, hsl(0 0% calc((var(--active) * 100%) + 65%)), hsl(0 0% calc((var(--active) * 100%) + 26%)))',
